...
class BratsDataset(Dataset):
    def __init__(self, opt, path_log, data_dir, normalization=True, isTrain=True, mod='single'):
        ...
        if mod == 'single':
            ...
            self.img_list = sorted(glob.glob(os.path.join(data_dir, "**", "GED4.npy"), recursive=True))
            self.mask_list = sorted(glob.glob(os.path.join(data_dir, "**", "mask_GED4.npy"), recursive=True))
            self.metadata_list = sorted(glob.glob(os.path.join(data_dir, "**", "metadata.json"), recursive=True))
            # self.img_list = self.img_list[1::3]
            # self.mask_list = self.mask_list[1::3]
            # self.metadata_list = self.metadata_list[1::3]
        elif mod == 'long':
    ...
    def load_file(self, filepath, if_image=True):
        ...
        if if_image:
            ...
            temp_image_t1ce = np.load(filepath)  # Load the .npy file

            # total_size = np.prod(temp_image_t1ce.shape)
            # if (total_size > 1e7):
            # print(f"The interpolate size : {temp_image_t1ce.shape}")
    ...
    def __getitem__(self, idx):
        ...
        if self.mod=='single':
            ...
            metadata_path = self.metadata_list[idx]
            # # Load the image and mask
            # # image, image_affine = self.load_file(image_path, if_image=True) ## [1, D, H, W]
            # image = self.load_file(image_path, if_image=True) ## [1, D, H, W]
            ...
            # mask = self.load_file(mask_path, if_image=False) ## [1, D, H, W]
            ...
            image = self.load_file(image_path, if_image=True)    # shape: [D, H, W] or [1, D, H, W]
            mask = self.load_file(mask_path, if_image=False)      # shape: [D, H, W] or [1, D, H, W]
            metadata = self.load_metadata(metadata_path)

            padding_info = metadata['padding_info']  # 形如 [[4,5], [6,6], [2,2]]
            # print(padding_info)
            ...
            original_shape = metadata['original_size']
            ...
            # 3. 转 Tensor
            image = torch.from_numpy(image).float()   # float32 for image
            mask = torch.from_numpy(mask).long()      # int label

            # 4. 返回值（你可以根据需要返回更多字段）
            return {
                'image': image,                       # Tensor, shape [1, D, H, W]
                'mask': mask,                         # Tensor, shape [D, H, W]
                'padding_info': padding_info,         # list of [before, after] per axis
                'original_spacing': original_spacing, # 原始 spacing
                'original_origin': original_origin,   # 原始 origin
                'original_direction': original_direction, # 原始 direction
                'original_shape': original_shape,     # 原始 shape
                'image_path': image_path[idx],       # 文件路径用于标识样本
                'mask_path': mask_path[idx]
            }
        else:
...
def custom_collate_fn(batch):
    ...
    batch_dict['image'] = torch.stack(batch_dict['image'])  # [B, 1, D, H, W]
    batch_dict['mask'] = torch.stack(batch_dict['mask'])    # [B, D, H, W]
    return batch_dict
