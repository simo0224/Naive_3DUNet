...
def _masked_mean_std(
  ...
  mask = 1 - pad
  ## mask & pad 的 shape 都是 (batch, patch_len)，也就是 (input.shape[0], input.shape[2])
  ## 也就是说现在只取的是一个patch的数据
  ...
  masked_sum = torch.sum(arr * mask, dim=1) ## arr 大小 [batch, patch]，
  masked_squared_sum = torch.sum((arr * mask)**2, dim=1)

  # Calculate the masked mean and standard deviation
  masked_mean = masked_sum / num_valid_elements
  masked_var = masked_squared_sum / num_valid_elements - masked_mean**2
  ## 这个是统计学上的方差计算公式：E(x^2)-[E(x)]^2，前面是平方的均值，后面是均值的平方
...
class TimesFMAttention(nn.Module):
  ...
  def __init__(
    ...
    # 使用正态分布初始化
    nn.init.normal_(self.scaling, mean=0.0, std=0.01)
  ...
  def _per_dim_scaling(self, query: torch.Tensor) -> torch.Tensor:
    ...
    # print(f"scale.nan: {torch.any(torch.isnan(scale))}, self.scaling: max: {torch.max(self.scaling.data)}; min: {torch.min(self.scaling.data)}") ## 这里出现 nan 了？
...
class StackedDecoder(nn.Module):
  ...
  def forward(
    ...
    for i in range(len(self.layers)):
      # if i >= 40:
      #   print("nan happens i==43, now i==40")
      layer = self.layers[i]
      ...
      if torch.isnan(hidden_states).any():
        raise ValueError(f"NaN detected in layer {i}")
...
class PatchedTimeSeriesDecoder(nn.Module):
  ...
  def __init__(self, config: TimesFMConfig=None):
    ...
    self.new_input_ff_layer = ResidualBlock(
        input_dims=2 * self.config.new_patch_len,
        output_dims=self.config.hidden_size,
        hidden_dims=self.config.intermediate_size,
    )

    self.freq_emb = nn.Embedding(num_embeddings=3,
                                 embedding_dim=self.config.hidden_size)
    self.new_horizon_ff_layer = ResidualBlock(
        input_dims=self.config.hidden_size,
        output_dims=self.config.horizon_len * (1 + len(self.config.quantiles)),
        hidden_dims=self.config.intermediate_size,
    )
  ...
  def _forward_transform(
    ...
    sigma = torch.where(
        ...
        sigma,
    ) ## 防止计算出的标准差（sigma）值太小，避免数值不稳定的问题，只是为了稳定计算，所以设置为 1 没什么不好
  ...
  def _preprocess_input(
    ...
    ## 以下两行是新添的，为了适应新更改的模型大小，做成了符合之前的输入的样子 [B, N*D*H*W, C], 也就是 [B, patch_num, patch_len]
    patched_inputs = input_ts
    patched_pads = input_padding
    ...
    patched_inputs = torch.where(
        ...
        patched_inputs,
    ) ## pads 值为 1 的时候对应把 patched_inputs 的值更改为 0
    patched_pads = torch.where(
        ...
        patched_pads,
    ) ## patched_inputs 的值是极大值的时候，把 pads 对应的值替换为 1,pads 为 1 的就是不能算的
    patched_inputs, stats = self._forward_transform(patched_inputs,
                                                    patched_pads) ## 这里是做一个均值化, stats是(u,sigma)

    # B x N x D, [B, num_patchs, patch_len]
    patched_inputs = patched_inputs * (1.0 - patched_pads) ## 这里剔除了异常值
    concat_inputs = torch.cat([patched_inputs, patched_pads], dim=-1) ## 最后一个维度连接，扩展 token_len
    model_input = self.new_input_ff_layer(concat_inputs) ## 与ptach_len无关
    ## concat_inputs: [729, 16, 64]
    ## model_input: [729, 16, 1280]
  ...
  def _postprocess_output(
    ...
    # B x N x (H.Q)
    output_ts = self.new_horizon_ff_layer(model_output)
  ...
  def decode(
    ...
    for step_index in range(num_decode_patches):
      ...
      if return_forecast_on_context and step_index == 0:
        ...
        new_full_ts = fprop_outputs[:, 0:-1, 0:self.config.new_patch_len, :]
    ...
    if return_forecast_on_context:
      ...
      full_outputs = torch.concatenate(
          full_outputs,
          axis=1)[:, :(context_len - self.config.new_patch_len + horizon_len), :]
    else:
